...
close all;
num = 99;
filename = strcat('jpg',num2str(num),'.png');
labelfilename = strcat('label',num2str(num),'.jpg');
...
[ReIm,Label,labelIm,Jaccard,Precision,Recall,Specificity,F1,Accuracy] = GetAnnoIm(Im,MD,labelIm,Jaccard,Precision,Recall,Specificity,F1,Accuracy);
% figure;hold on;
...
putput = "Jaccard："+Jaccard+"  "+"Precision："+Precision+"  "+"Recall："+Recall+"  "+"Specificity："+Specificity+"    "+"F1："+F1+"  "+"Accuracy："+Accuracy;
title(putput);
set(g,'Visible','on');
...
function [ReloadIm,P_Label,labelIm,Jaccard,Precision,Recall,Specificity,F1,Accuracy] = GetAnnoIm(Im,Mdl,labelIm,Jaccard,Precision,Recall,Specificity,F1,Accuracy)
RGBvec = RGB2vec(Im);
P_Label = predict(Mdl,RGBvec);
P_Label = (P_Label==1);% 1对应云
Jaccard = 0;
Accuracy = 0;
% Reload to image
[row,col,~] = size(Im);
